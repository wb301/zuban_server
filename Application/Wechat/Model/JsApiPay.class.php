...
class JsApiPay
{
    ...
    {
        if(empty($config)) {
            throw new WxPayException('配置参数不能为空');
        }
        $this->config = $config;
    }
    ...
    public function GetOpenid($baseUrl='')
    {
        ...
        if (!isset($_GET['code'])){
            ...
            Log::record('wechat url:'.$url,Log::DEBUG,true);
            Header("Location: $url");
            exit();
        } else {
            //获取code码，以获取openid
    ...
    {
        $wxUrl = $this->__CreateOauthUrlForCode($url);
        return $wxUrl;
    }
    ...
    public function GetOpenidFromMp($code)
    {
        ...
        $proxyHost = $this->config->getCurlProxyHost();
        $proxyPort = $this->config->getCurlProxyPort();
